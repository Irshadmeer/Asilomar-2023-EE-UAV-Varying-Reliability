...
def main(algorithm, output, num_tries, max_coordinates, plot: bool = False, **kwargs):
    bs_locations = np.array(
        [
            (random.uniform(0, 3000), random.uniform(0, 3000), 25)
            for _ in range(kwargs["num_base_stations"])
        ]
    )
    ...
    model = func_algorithm(
        "MultiInputPolicy", env, verbose=1, tensorboard_log="./tensorboard_log/"
    )
    tb_callback = SummaryWriterCallback()
    df_callback = StoreDataCallback()
    callbacks = [df_callback, tb_callback]
    model.learn(total_timesteps=num_tries, progress_bar=True, callback=callbacks)
    ...
    output = os.path.join(
        os.path.dirname(output), "{}_{}".format(algorithm, os.path.basename(output))
    )
    model.save(output)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--reliability_constraint", type=float, default=0.00001)
