...
class BasicScenario(gym.Env):
    def __init__(self, num_users, num_base_stations,
    ...
    def reset(self):
        ...
        self.movement_state = [{"x": [0, 0, 0],
                                "y": [0, 0, 0]}
                               for _ in range(self.num_users)]
        user_locations = np.tile(np.array(self.max_coordinates)/2, (self.num_users, 1))
        user_locations[:, 2] = (.5+.5*np.random.rand(self.num_users))*self.max_coordinates[2]
    ...
    def _get_new_state(self):
        ...
        for user in range(self.num_users):
            ...
            new_locations[user, 0] = scale*_new_state_x[0] + self.max_coordinates[0]/2
            new_locations[user, 1] = scale*_new_state_y[0] + self.max_coordinates[1]/2
    ...
    def step(self, action):
        user_locations = self.state["user_locations"]
        base_station_powers = 10**(self.max_power_db*action/10.)  # convert dB power (action) in linear scale
        los = self.state['los']
        ...
        outage_probabilities = calculate_outage_probabilities(
                user_locations, self.bs_locations, base_station_powers, 
                self.freq, self.rec_sensitivity, los)

        total_power = np.sum(base_station_powers)
       
        total_power_per_user =np.sum(base_station_powers, axis=1)/np.sum(max_power,axis=1)
        total_power = np.sum(base_station_powers)
        ...
        reward_active_bs   = 1-(num_active_connections/(self.num_users*self.num_base_stations))
    ...
    def is_done(self):
        ...
        if np.any(new_user_locations < 0) or np.any(new_user_locations > self.max_coordinates):
            return True
        return False
